...
bool Application::GetMouseUpdate()
{
	...
	else if ((mouse_current_x > m_window_width - m_window_deadzone))
	{
	...
	else if (mouse_current_y > m_window_height - m_window_deadzone)
	{
...
void Application::Init()
{
	...
	m_window = glfwCreateWindow(m_window_width, m_window_height, "The Maze", NULL, NULL);
...
void Application::Run()
{
	...
	Game_Core::pointer()->Init();
	m_timer.startTimer();    // Start timer to calculate how long it takes to render this frame

	while (!glfwWindowShouldClose(m_window) && ! Application::IsKeyPressed(VK_ESCAPE))
	{
		GetMouseUpdate();
		Game_Core::pointer()->Update(m_timer.getElapsedTime());
		Game_Core::pointer()->Render();
		//Swap buffers
		glfwSwapBuffers(m_window);
		//Get and organize events, like keyboard and mouse input, window resizing, etc...
		glfwPollEvents();

		//std::cout << float(1 / m_timer.getElapsedTime()) << std::endl;

        m_timer.waitUntil(frameTime);       // Frame rate limiter. Limits each frame to a specified time in ms.   

	} //Check if the ESC key had been pressed or if the window had been closed

	Game_Core::pointer()->Exit();
}
